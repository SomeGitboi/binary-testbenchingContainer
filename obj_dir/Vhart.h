...
#ifndef VERILATED_VHART_H_
#define VERILATED_VHART_H_  // guard

#include "verilated.h"

class Vhart__Syms;
class Vhart___024root;

// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) Vhart VL_NOT_FINAL : public VerilatedModel {
  private:
    // Symbol table holding complete model state (owned by this class)
    Vhart__Syms* const vlSymsp;
  ...
  public:
    ...
    explicit Vhart(VerilatedContext* contextp, const char* name = "TOP");
    explicit Vhart(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~Vhart();
  private:
    VL_UNCOPYABLE(Vhart);  ///< Copying not allowed
